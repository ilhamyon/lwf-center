...
const columns = [
  ...
  },
  {
    ...
    render: (item, record) => (
      <div>
        {record.dusun}, {record.village}, {record.district}
      </div>
    )
  },
  {
  ...
  {
    title: 'Telepon',
    dataIndex: 'teleponPenanggungJawab',
    key: 'teleponPenanggungJawab',
  },
  {
    title: 'Bentuk Bantuan',
    dataIndex: 'bentukBantuan',
    key: 'bentukBantuan',
  },
  {
...
function Home() {
  ...
  const lfwCenter_user = (localStorage.getItem('lfwCenter_user'));
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        setIsLoading(true);
        const sanityData = await sanityClient.fetch(`*[_type == 'lfw-entry']{
          _id, jenisKegiatan, date, province, regency, district, village, dusun, lokasiSpesifik, penanggungJawab, teleponPenanggungJawab, bentukBantuan, jumlahPeserta, keterangan, fotoEksternal, "foto": foto.asset->url, geometry, user-> {name}
        }`);
  ...
  useEffect(() => {
    const initializeMap = async () => {
      const data = serverData && serverData.data && serverData.data.length > 0
        ? serverData.data.filter(item => item.user.name === lfwCenter_user)
        : [];
      ...
      const updatedDataSource = data.map((item) => ({
        ...
        teleponPenanggungJawab: item.teleponPenanggungJawab || "-",
        lokasiSpesifik: item.lokasiSpesifik || "-",
        bentukBantuan: item.bentukBantuan || "-",
        dusun: item.dusun || "-",
  ...
  const rearrangeDataForExcel = () => {
    ...
    const rearrangedData = dataSource.map((item, index) => {
      return {
        ...
        'Lokasi': `${item.dusun}, ${item.village}, ${item.district}`,
        'Lokasi Spesifik': item.lokasiSpesifik,
        'Jenis Kegiatan': item.jenisKegiatan,
        'Penanggung Jawab': item.penanggungJawab,
        'Telepon Penanggung Jawab': item.teleponPenanggungJawab,
        'Bentuk Bantuan': item.bentukBantuan,
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `lfw-entry-${volunteerName}.xlsx`);
  };
        ...
        <div className="relative mx-auto max-w-screen-xl px-4 py-32 sm:px-6 lg:flex lg:h-screen lg:items-center lg:px-8">
          <div className="max-w-xl text-center sm:text-left">
            ...
            <p className="mt-4 max-w-lg sm:text-xl/relaxed text-gray-700">
              Daftar rekap data kegiatan dan kunjungan.
            </p>
